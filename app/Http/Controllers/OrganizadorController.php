    ...
    public function updateProfileForm(Request $request)
    {
        ...
        try {
            ...
            if ($request->filled('senha_atual')) {
                if (!password_verify($request->senha_atual, $organizador->password)) {
                    return back()->withInput()->withErrors(['senha_atual' => 'Senha atual incorreta.']);
                }
                
                // Atualiza a senha
                if ($request->filled('senha_nova')) {
                    $organizador->password = bcrypt($request->senha_nova);
                }
            }
            
            // Atualiza dados básicos do organizador (usando os nomes corretos das colunas)
            $organizador->name = $request->nome;
